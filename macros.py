...
def testing(filename):
    text = demacro_file(filename)
    with open('/media/jay/Data/current_debug.tex','w') as fh:
        fh.write('%'+filename+'\n')
        fh.write(text)
...
def testing3():
    ...
    global diag_message
    diag_message = True
    print(demacro_file('macroexamples.tex'))
    diag_message = False
    end = time.time()
...
def multithreaded():
    ...
    folderlist = [os.path.join(path,item) for item in folderlist]
    pool = mp.Pool(mp.cpu_count())
    pool.map(demacro_archive,folderlist)
    pool.close()
    pool.join()
...
def testing6(path):
    ...
    folderlist = next(os.walk(path))[1]
    folderlist = [os.path.join(path,item) for item in folderlist]
    outdirectory = '/media/jay/Data/'+folder_name+'_out'
    print("CURRENT outdirectory: {}".format(outdirectory))
    validate_folder(outdirectory)
    print("{} folders".format(len(folderlist)))
    pool = mp.Pool(mp.cpu_count())
    pool.map(mapped,folderlist)
...
# example1 = "a#1 should stay intact, but a##2 should be reduced"
# example2 = "a##1 is reduced, and we should preserve a\\#####4"
